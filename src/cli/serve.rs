...
//! applies the snapshot in two steps (kernel state first, then linear
//! memory — they cannot share a `Store` borrow, see
//! `src/snapshot.rs:679-681`), then respawns the guest at the
//! post-snapshot state. The wasm module path is required because
//! `KernelSnapshot` does not carry module bytes (`src/snapshot.rs:158-187`).
//!
//! `--port <p>` pre-mutates `snap.fds[].kind.body.socket.bound.port` for
//! any IPv4 listener fd so the reopen path at `src/snapshot.rs:879`
//! binds to the override port. (Re-binding post-apply would require
//! holding two listeners briefly; pre-mutating is simpler.)
//!
...
use crate::snapshot::{
    apply_snapshot_kernel_state, apply_snapshot_to_memory, read_snapshot_file, KernelSnapshot,
};
...
pub async fn run_main(args: &[String]) -> CliResult<i32> {
    ...
    serve_loop(&snap, wasm_path).await
}
...
async fn serve_loop(snap: &KernelSnapshot, wasm_path: &str) -> CliResult<i32> {
    ...
    // Two-step apply (per `src/snapshot.rs:679-681`):
    //   1. apply_snapshot_kernel_state — no Store borrow; takes &mut Kernel.
    //   2. apply_snapshot_to_memory — needs &mut Store<Kernel>; we clone
    //      the Memory handle out of the kernel first (Memory is Copy).
