//! `edge-cli bench <snap> <wasm> --iters <n>`.
//!
...
pub async fn run_main(args: &[String]) -> CliResult<i32> {
    ...
    if positional.len() < 2 {
        return Err(CliError::Args(
            "usage: edge-cli bench <snap> <wasm> [--iters <n>]".to_string(),
        ));
    }
    ...
    for i in 0..iters {
        ...
        let _ = instance; // bench does not call _start — apply only
        ...
        if i % 10 == 0 {
            eprintln!("  iter {i:>3}: {elapsed_us} µs");
        }
    }
